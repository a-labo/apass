...
class APass {
   ...
   * @param {string} remote - Remote repository name
   * @returns {Promise}
   */
  bind (remote) {
    const remoteAs = 'origin'
    const s = this
    return co(function * () {
      if (!remote) {
        remote = yield s.ask('Remote git repository')
      }
      if (!remote) {
      ...
      yield s.execGit('commit', '-m', '[apass] Share secrets')
      yield s.execGit('remote', 'add', remoteAs, remote)
      yield s.execGit('push', '-u', remoteAs, 'master')
    })
  }
